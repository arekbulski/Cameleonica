...
for area in map(BytesInt, '1MB 4MB 16MB 128MB 1GB 4GB 16GB 128GB 1TB'.split()):
    ...
    if displaytimes:
        print 'Read times: {0} ... {1} ms'.format(' '.join(['{0:0.2f}'.format(x*1000) for x in times[:displaysamplesize/2]]), ' '.join(['{0:0.2f}'.format(x*1000) for x in times[-displaysamplesize/2:]]))
